(function ( $ ) {
	...
	$( function() {
		$( 'div.wpcf7' )
    .on( 'submit', 'form.wpcf7-form', function() {
			$( this ).find( 'button.wpcf7-submit' ).addClass( 'disabled' ).prop( 'disabled', true );
      return true;
		})
    .on( 'wpcf7:submit', function() {
			$( this ).find( 'button.wpcf7-submit' ).removeClass( 'disabled' ).prop( 'disabled', false );
		})
		...
		return this.each( function() {
			...
			if ( ! $into.parents( '.wpcf7-form' ).hasClass( 'form-inline' ) ) {
				...
				if ( $into.parents( '.wpcf7-form' ).hasClass( 'form-horizontal' ) ) {
					...
					$into.append( '<span class="form-control-feedback wpcf7-not-valid-tip">' + message + '</span>' );
				}
				$into.slideDown( 'fast' );
	...
	$.fn.wpcf7ClearResponseOutput = function() {
		return this.each(function() {
			...
			$( this ).find( 'div.form-group' ).removeClass( 'has-danger' );
			$( this ).find( 'span.wpcf7-not-valid-tip' ).remove();
		});
	};

  $(document).ready( function () {
    if ( $( '.input-group.range-slider' ).length ) {
      $( '.input-group.range-slider input.wpcf7-form-control' ).each( function() {
        $( this ).on( 'slide', function (e) {
          var arialabeledby = $(this).attr('id');
          $( '#range-addon-'+arialabeledby ).text( e.value );
        });
      });
    }
  });
