...
module.exports = panels.view.dialog.extend( {
				...
				handle.draggable( {
					...
					drag: function ( e, ui ) {
						...
						$( this ).data( 'newCellClone' ).css( 'width', rowPreview.width() * ncw )
							.find( '.preview-cell-weight' ).addClass('dragged').html( Math.round( ncw * 1000 ) / 10 );

						$( this ).data( 'prevCellClone' ).css( 'width', rowPreview.width() * pcw )
							.find( '.preview-cell-weight' ).addClass('dragged').html( Math.round( pcw * 1000 ) / 10 );
					},
					stop: function ( e, ui ) {
						// Remove the clones
            var ncw, pcw;
              ...
              ncw = thisDialog.getClosestWidth( (thisDialog.row.cells[i] - percent) );
              pcw = thisDialog.getClosestWidth( (thisDialog.row.cells[i - 1] + percent) );
						} else {
              ncw = thisDialog.row.cells[i];
              pcw = thisDialog.row.cells[i - 1];
            }

            thisDialog.row.cells[i] = ncw / 100;
						thisDialog.row.cells[i - 1] = pcw / 100;
	...
	scaleRowWidths: function () {
      ...
      var col = thisDialog.row.cells[i] * 100,
          colC = Math.round( col * 12 ) / 100,
          colT = colC > 1 ? colC+' Columns' : colC+' Column';
			$( el )
				.css( 'width', thisDialog.row.cells[i] * 100 + "%" )
				.find( '.preview-cell-weight' ).removeClass('dragged').html( colT );
		} );
	},
	...
	setCellsFromForm: function () {
		...
		try {
          ...
          var colW = thisDialog.row.cells[i] * 100,
              colName = Math.round( colW * 12 ) / 100,
              colText = colName > 1 ? colName+' Columns' : colName+' Column';
					$( el ).animate( {'width': Math.round( thisDialog.row.cells[i] * 1000 ) / 10 + "%"}, 250 );
					$( el ).find( '.preview-cell-weight' ).html( colText );
				} );
  ...
  /**
	 * Get Closest Cell Width from predefined Column Width
	 */
	getClosestWidth: function (num) {
    var array = [ 8.333333, 16.666667, 25, 33.333333, 41.666667, 50, 58.333333, 66.666667, 75, 83.333333, 91.666667, 100 ];
    var i = 0;
    var minDiff = 101;
    var rounded = num*100;
    var ans;
    for(i in array){
      var m = Math.abs(rounded-array[i]);
      if (m < minDiff) {
        minDiff = m;
        ans = array[i];
      }
...
module.exports = Backbone.View.extend( {
	...
	initResizable: function () {
		...
		handle.draggable( {
          ...
          ncw = cellView.getClosestWidth( ncw );
          pcw = cellView.getClosestWidth( pcw );
					cellView.row.builder.addHistoryEntry( 'cell_resized' );
					cellView.model.set( 'weight', (ncw / 100) );
					previousCell.model.set( 'weight', (pcw / 100) );
					cellView.row.resize();
				}
